...
{
  uint32_t timer_cnt = __HAL_TIM_GET_COUNTER(&htim3);
	
	elapsed_time += timer_cnt - last_counter_val;
	last_counter_val = htim3.Init.Period - value;
	
	__HAL_TIM_SET_COUNTER(&htim3, htim3.Init.Period - value);
