...
/* index 0x1006 :   Communication / Cycle Period. */
                    UNS32 TestMaster_obj1006 = 1000000;	/* 50000 */
...
UNS8 GetChangeStateResults(UNS8 node_id, UNS8 expected_state, unsigned long timeout_ms)
 ...
 // reset nodes state
 TestMaster_Data.NMTable[node_id] = Unknown_state;

 // request node state
 masterRequestNodeState(&TestMaster_Data, node_id);
 
 start_time = HAL_GetTick();
 while(HAL_GetTick() - start_time < timeout_ms)
		{
		if (getNodeState(&TestMaster_Data, node_id) == expected_state)
			return 0;
			osDelay(1);  
		}
 return 0xFF;
